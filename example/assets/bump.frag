...
void main()
{
	...
	LOWP vec4 texture = texture2D (u_texture0, v_texCoords0);
	LOWP vec3 normal = texture2D (u_texture1, v_texCoords1).xyz * 2.0 - 1.0;
	float att = 1.0 / (1.0 + u_attenuation * lightDist2);
	LOWP vec3 diffuse = vec3(min(vec3(1.0), (max(0.0, dot(normal, v_lightDir)) * att + u_ambient))) * texture.rgb;
